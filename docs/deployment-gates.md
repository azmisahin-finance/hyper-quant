...
## v2.8 execution-safety release gates
...
Authorization snapshots are single-use, hash-bound, scope-aware, and version-bound. Expiry is checked against a fresh authoritative clock reading at every validation point. The final revalidation must occur immediately before the commit boundary, and the physical mutation slot remains exclusive. Test-only deterministic barriers may control scheduling but never act as a safety authority.
