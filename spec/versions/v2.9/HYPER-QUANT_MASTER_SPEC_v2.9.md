# HYPER-QUANT MASTER SPEC v2.9
## Quant Research, Capital Growth, Risk, Multi-Venue Data & Execution Platform

**Status:** MASTER SPECIFICATION — REVIEW_REQUIRED / v2.9 HARDENING / INDEPENDENT ADVERSARIAL REVIEW REQUIRED

**Revision:** v2.9 is a hardening amendment to v2.8. It converts the previously scaffolded execution-safety contracts into a stricter executable boundary: scope-aware kill applicability, hashed authorization snapshots, fresh-time expiry validation, explicit hard-risk/dependency/policy gates, canonical intent transitions, bounded reconciliation, and a durable append-only intent journal. It retains v2.8 as the prior review artifact and does not grant production approval.
**Version:** 2.9
**Date:** 2026-09-08
**Supersedes as strategic authority:** HYPER-QUANT v2.8; v2.7; v2.6; v2.0; v1.5 remains normative only where explicitly retained as the Safety/Execution Kernel
...
**v2.9 hardening amendments:** this version retains all v2.8 normative controls and extends the executable reference boundary into research integrity, statistical validation, artifact identity, holdout-budget accounting, promotion policy, aggregate exposure, deterministic chaos coverage, and test/production capability parity. All such implementations remain non-live reference controls; venue adapters and signing remain outside production authority.
...
## 9.7 v2.9 Executable Safety-Kernel Alignment

The v2.9 implementation is not permitted to weaken the v2.7 intent lifecycle by introducing an alternate execution state machine. The canonical state set is implemented by `src/execution/intent-state-machine.ts` and all transitions must pass the same transition table.
...
## 39.1.1 v2.9 Kill Applicability Record
...
## 20.11 Executable Research Integrity Reference Controls

The repository shall provide deterministic, fail-closed reference controls for executable feature dependency derivation, artifact/dependency identity binding, non-resettable holdout-budget accounting, and statistical acceptance evidence. These controls are governance primitives and do not constitute proof of venue economics.

### 20.11.1 Research Dependency Manifest

The approved research runner shall derive a dependency manifest from typed executable feature nodes. The manifest shall bind feature identifiers, input columns, maximum extracted lookback, label horizon, purge window, extractor version, and analysis configuration hash. A researcher-declared lookback below the extracted bound is a hard `RESEARCH_INTEGRITY_KILL` condition.

### 20.11.2 Artifact Identity Reference Check

The repository shall expose an executable identity chain covering source tree, artifact, manifest, dependency lock, dependency graph, build environment, and executable artifact hashes. Execution against a mismatched registered chain shall fail closed.

### 20.11.3 Holdout Ledger Reference Control

The repository shall provide an append-only holdout ledger primitive with a non-recreatable governed program root, monotonic global/family/lineage consumption, serialized writes, and explicit result classes. Raw holdout rows, labels, timestamps, feature dumps, and adaptive oracle payloads shall not cross the evaluation boundary.
...
## 21.5 Executable Statistical Acceptance Gate

The repository shall provide a deterministic reference gate that verifies declared trial count, minimum sample policy, positive lower confidence bound for mean trade expectancy under the declared estimator, and the pre-declared Deflated Sharpe Ratio threshold. Analyst-supplied trial counts are not authoritative outside the committed research ledger.
...
## 23.6 Executable Aggregate Exposure Gate

The repository shall provide a deterministic aggregate exposure evaluator that independently checks approved, allocated, deployed, gross, net, margin, liquidity, concentration, correlation-adjusted, and tail-loss dimensions. Any non-finite or unknown dimension fails closed.
...
## 25.5 Executable Promotion Policy

Promotion shall be evaluated by a frozen policy artifact. Level skipping is forbidden. Evidence, dwell, observation, promotion velocity, allocation velocity, safety status, calibration, regime coverage, and aggregate-risk gates are independently required. The reference policy caps allocation growth at 2x the prior allocation.
...
## 40.6 Executable Chaos Registry and Capability Parity

The repository shall validate a machine-readable chaos registry for unique scenario identities and required evidence fields, and shall reject production capability manifests containing test-only capabilities or mismatched safety capability hashes. These are hard release-gate controls.
...
| 11–16 Feature, regime, alpha and probability layers | REVIEW_REQUIRED | Quant / AI Researcher / Trader | deterministic fixtures + leakage tests + model artifact tests | Next adversarial review required for v2.9 |
| 17–21 Statistical validation and promotion gates | REVIEW_REQUIRED | Quant / Investor | replay, OOS, walk-forward, PBO/CSCV, DSR and multiple-testing audit | Next adversarial review required for v2.9 |
| 22–24 Decision and Risk interfaces/aggregation | REVIEW_REQUIRED | Risk Manager / Architect | type/schema compatibility tests + risk aggregation property tests | Next adversarial review required for v2.9 |
| 25–29 Capital, AI governance and isolation | REVIEW_REQUIRED | Risk / Security / AI Researcher | sandbox/network/filesystem denial tests + lineage tests | Next adversarial review required for v2.9 |
...
| 37–40 Venue protection, failure taxonomy, kill-switch, chaos | REVIEW_REQUIRED | Trader / Security / Senior Engineer | venue capability tests + 46 chaos scenarios + state assertions | Next adversarial review required for v2.9 |
...
| 49–54 Repository, authority, phases, acceptance, deliverables, invariants | REVIEW_REQUIRED | Senior Engineer / Architect | implementation traceability + static review | Next adversarial review required for v2.9 |
| 55 Review Status Matrix | REVIEW_REQUIRED | Architect / Senior Engineer | self-consistency audit + release-build state-integrity checks | v2.9 hardening state changed; next adversarial review required |
| 56 Pre-Implementation Decision Pack | REVIEW_REQUIRED | CEO / Quant / Risk / Trader | signed decision record + amendment control | v2.9 retains the v2.7 research priority and adds executable safety-kernel hardening |
| 57 Definition of Done | REVIEW_REQUIRED | Architect / CEO | release checklist + traceability | v2.9 hardening and implementation traceability require adversarial verification |
...
| 59 Iterative Review Closure Protocol | REVIEW_REQUIRED | All personas | review-record audit + independent re-review | This protocol governs the v2.9 hardening review cycle itself |
...
**Overall v2.9 matrix status:** `REVIEW_REQUIRED`. v2.9 is not specification-approved. No implementation claim may infer approval from a partial row-level review.

# 56. Pre-Implementation Decision Pack

The following decisions are fixed as the v2.9 baseline so that implementation agents do not invent them; v2.7 remains the preceding immutable review artifact. Any later change requires a versioned amendment with independent review.
...
# 57. Definition of Done for v2.9

HYPER-QUANT v2.9 is ready for the next implementation phase only when:
...
- research-trial registration, research integrity, statistical, holdout, exposure, promotion, chaos, and capability-parity reference controls pass deterministic verification.

HYPER-QUANT v2.9 is ready for **real capital** only after a separate production gate passes.
...
**v2.9 AS RESEARCH + VALIDATION + CAPITAL-GROWTH PLATFORM: REVIEW CANDIDATE; EXECUTION-SAFETY SUBSET HARDENED, RESEARCH/CAPITAL/LIVE VENUE IMPLEMENTATION STILL PENDING**
...
# END OF MASTER SPECIFICATION v2.9
